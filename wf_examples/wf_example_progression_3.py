...
"""Example 3 — Interactly Workflow SDK.
...
See ``wf_example_progression_3.md`` for an illustrated walkthrough — a schematic
diagram, node/edge tables, key details, and a sample conversation.

Run it::

    INTERACTLY_API_KEY=... python wf_examples/wf_example_progression_3.py
"""
...
def build_assistant_workflow():
    ...
    google_docs_md_link = (
        "https://docs.google.com/document/d/1nYFTeDCnDPS5z91yKzgaYNlL2Ew_sl2TXb5QYc0Zjfg/edit?tab=t.v81qysrcwp02"
    )
    ...
    workflow_config = WorkflowConfig(
        ...
        name="Example 3:  Branching ChatBot with more flavors",
    ...
    Trigger this to come back to the internal routing agent when the user has clearly switched their intent in the middle of the conversation and want to do something different.

    For example, if you are a scheduling assistant, and the user wants help with something else like insurance, take this path.
    Examples of such user inputs include:
    - "Actually, I have a question about my insurance."   [E.g when the user was previously talking to the scheduling assistant]
    - "I need help with scheduling, but first I have an insurance question."  [E.g when the user was previously talking to the payment assistant]
    - "Can you help me with payments instead?" [E.g when the user was previously talking to the insurance assistant]
    ...
    router_node = SayLLMNodeConfig(
        ...
        global_node_config=GlobalNodeConfig(
            is_global=True, condition=ConditionConfig(condition_freeform=ROUTER_GLOBAL_EDGE_CONDITION)
        ),
    )
    ...
    **Example 3 - Irrelevant Question**
    ...
    end_conversation_node = SayStaticMessageNodeConfig(
        ...
        static_messages_config=StaticMessagesConfig(static_messages=["{{farewell_phrase_1}}", "{{farewell_phrase_2}}"]),
        global_node_config=GlobalNodeConfig(
    ...
    Take this path to route to an internal agent that will help with insurance questions. Take this path only when it is clear that the user wants help with insurance.
    """
    ...
    Take this path to route to an internal agent that will help with scheduling questions. Take this path only when it is clear that the user wants help with scheduling.
    """
    ...
    Take this path to route to an internal agent that will help with payment questions. Take this path only when it is clear that the user wants help with payments.
    """
