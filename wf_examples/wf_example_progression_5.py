...
"""Example 5 — Interactly Workflow SDK.
...
See ``wf_example_progression_5.md`` for an illustrated walkthrough — a schematic
diagram, node/edge tables, key details, and a sample conversation.

Run it::

    INTERACTLY_API_KEY=... python wf_examples/wf_example_progression_5.py
"""
...
def build_assistant_workflow():
    ...
    google_docs_md_link = (
        "https://docs.google.com/document/d/1nYFTeDCnDPS5z91yKzgaYNlL2Ew_sl2TXb5QYc0Zjfg/edit?tab=t.med4xab64a2k"
    )
    ...
    workflow_config = WorkflowConfig(
        ...
        name="Example 5: Expression-Based Routing with Worker LLM",
    ...
    summary_node = SayLLMNodeConfig(
        ...
        self_loop=False,
        # wait_for_user_message=True: the summary node responds immediately after the worker
        # transitions to it, then waits for the user to acknowledge before moving to the end node.
        # This matches Example 4's behaviour — the user gets a chance to confirm the summary.
        wait_for_user_message=True,
    ...
    INTAKE_TO_SUMMARY_CONDITION_EXPRESSION = "isPresent([[full_name]]) AND isPresent([[primary_complaint]])"
    ...
    intake_to_summary_edge = ConditionalEdgeConfig(
        ...
        condition=ConditionConfig(condition_expression=INTAKE_TO_SUMMARY_CONDITION_EXPRESSION),
    )
